...
| `src/lib/physicalInvoiceDesk.ts`        | Paper-invoice promise window (Mon–Fri 08:00–17:00 Asia/Manila), mirroring the API's `PATCH /orders/:id/physical-invoice` check; the workspace offers only times still ahead. `physicalInvoiceRequest` is Ops/Super Admin and the owning client only — the API strips it for shops and riders
...
| `src/app/admin/tracker/` + `src/app/admin/_lib/tracker.ts` | Super Admin Tracker: the captain's sheet read live from GitHub issues labelled `tracker` (API contract `gridgo-api/docs/TRACKER_API.md`). Sections/statuses/filters/decision limits live in `_lib/tracker.ts`. Only Status is editable (confirmed, optional audit note); a Needs decision item opens the Decision panel (text + up to 6 PNG/JPEG/WebP/PDF of 10 MB, uploaded as `tracker_decision` only on save; attachment links are signed and fetched on click). `503 tracker_not_configured` is a calm "not connected yet" state, never an error. Sheet IDs repeat per section, so control names carry the requirement (`trackerItemName`). Screenshots: `docs/screenshots/tracker/` |
| `src/components/issue-reports/`        | Issue reports desk (`/ops/issue-reports`, `/admin/issue-reports`) for the website's public Report an issue form (`gridgo-api/docs/ISSUE_REPORTS_API.md`). Statuses `new · tracked · published · dismissed`; `tracked` carries `trackerIssueUrl`, checked by `tracker-link.ts` against `https://github.com/gridgoph/<repo>/issues/<n>` before sending (the API refuses anything else) and shown as a `StatusChipLink` ("Tracked: gridgo-web #62", new tab). The link survives `published` and is cleared by the API on `new`/`dismissed`. Screenshots: `docs/screenshots/issue-reports/` |
...
**Single source:** `src/lib/nav.ts` → `ROLE_NAV_GROUPS` (rail sections) and flattened `ROLE_NAV` (items). Overview / Jobs stay top-level as plain rows; every labeled group (Queue / Field / Money / System for ops; People / Catalog / Money / System for admin; Shop / Money for suppliers) is a collapsible parent row — group `icon` + label + chevron — with its pages as indented sub-rows on a guide line. The group holding the current page opens itself; other groups remember the person's choice, and the whole-rail fold is remembered too (`src/components/shell/rail-state.ts`, localStorage). In the icon rail a group opens its pages in a flyout. Small muted section headings (sidebar-07) sit above runs of groups — each `NavGroup.section`; ops Desk / Work / Platform, admin Desk / Manage / Platform, supplier Work / Business — and drop out on the icon rail. Count badges sit inline right after a label, never at the far edge: `NavItem.count` names the source, `src/lib/nav-counts.ts` owns sums, wording ("Orders, 3 need action" as the row's `aria-label`) and 99+, and `src/lib/live/useNavCounts.tsx` reads each source once per rail from an existing list endpoint (never a rail-only endpoint). Yellow only for orders waiting on Operations; other counts are monochrome. A folded group shows its filled total; an open group shows an outlined total only when two or more rows are counted; the icon rail puts the total on the group icon and per-page counts in the flyout. Screenshots: `docs/screenshots/sidebar/`.
...
| super_admin | `/admin/overview`, `riders`, `verification`, `roles`, `catalogue`, `zones`, `credits`, `finance`, `settings`, `audit`, `planning`, `tracker`, `broadcast` |
