...
describe("Clerk session middleware", () => {
  it.each(["/", "/supplier/jobs", "/ops/orders", "/admin/roles"])(
  ...
  it("keeps login public and redirects signed-out role routes without consulting claims", async () => {
    ...
    const request = new NextRequest("https://portal.example/ops/orders");

    const response = await portalSessionMiddleware(auth, request);

    expect(response.status).toBe(307);
    expect(response.headers.get("location")).toBe(
      "https://portal.example/login?redirect_url=https%3A%2F%2Fportal.example%2Fops%2Forders",
    );
  ...
  it("rebuilds the public login bounce from forwarded host when the container bind address is visible", async () => {
    ...
    const request = new NextRequest("http://0.0.0.0:3000/ops/orders", {
      headers: {
    ...
    expect(response.status).toBe(307);
    expect(response.headers.get("location")).toBe(
      "https://gridgo-dash.talasora.com/login?redirect_url=https%3A%2F%2Fgridgo-dash.talasora.com%2Fops%2Forders",
    );
  });
