...
import { actionsForOps, opsHandoffForState, type OpsAction } from "@/lib/ops-actions";
import {
...
export default function OpsQaWorkspacePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setOrder(next);
      setPeople(
        Object.fromEntries(directory.map((u) => [u.id, u.supplierName || u.name])),
      );
    } catch (err) {
      setOrder(null);
  ...
  if (loading && !order) return <SkeletonDetail label="Loading QA workspace" />;
  if (error || !order) {
    return (
      <ErrorState
          ...
          <div className="min-w-0">
            ...
            {order.assignmentNotifiedAt ? (
              <p className="text-caption text-text-muted m-0 mt-1">
                Client told the final price {formatDateTime(order.assignmentNotifiedAt)}
              </p>
            ) : null}
          </div>
          ...
          {primary || secondary.length ? (
            <>
              ...
              {primary?.requires === "supplierId" ? (
                <p className="text-caption text-text-muted m-0">
                  Assigns the demo supplier. Use Matching to choose from eligible,
                  approved suppliers with a reason.
                </p>
              ) : null}
              {primary?.requires === "riderId" ? (
                <p className="text-caption text-text-muted m-0">
                  Assigns the demo rider when none is set. Riders normally accept an offer
                  themselves from the rider app.
                </p>
              ) : null}
            </>
          ) : handoff ? (
            <div className="flex flex-wrap items-center gap-3">
              <p className="text-body text-text-secondary m-0">
                Nothing to change here — this order&rsquo;s next step lives on another
                surface.
              </p>
              <Button
                variant="secondary"
            ...
            <p className="text-body text-text-secondary m-0">
              Nothing for Operations to change from here. The timeline below shows who
              moved this order last, and when.
            </p>
          )}
          {actionError ? (
      ...
      <div className="grid w-full gap-3 lg:grid-cols-2 lg:items-start">
          ...
          <dl className="mt-4 grid grid-cols-1 gap-3 border-t border-outline-subtle pt-4 sm:grid-cols-2">
            <div>
              ...
              <dd className="text-body text-text-primary m-0">
                {order.supplierId
                  ? (people[order.supplierId] ?? presentTimelineActor(order.supplierId))
                  : "Not assigned"}
              </dd>
            </div>
            <div>
        ...
        <section className="gg-card p-3" aria-labelledby="money-heading">
          ...
          <p className="text-body text-text-secondary m-0 mt-1 mb-3 max-w-prose">
            Operations and Super Admin only. The client is never shown the supplier price
            or the commission.
          </p>
        ...
        {checklist && checklist.status !== "not_started" ? (
          <section className="gg-card p-3" aria-labelledby="pickup-heading">
            ...
            {pickupEvidence(order).length ? (
              <div className="mt-3">
                <EvidenceStrip items={pickupEvidence(order)} />
              </div>
            ) : null}
