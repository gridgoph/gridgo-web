    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat
          ...
          loading={pending}
          value={String(snapshot?.verifiedSupplierCount ?? 0)}
        />
        <Stat
          label="Verified riders"
          loading={pending}
          value={String(snapshot?.verifiedRiderCount ?? 0)}
        />
      </div>
      ...
      <section className="gg-card" aria-labelledby="week-heading">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-body text-text-secondary m-0 mt-1">
              {formatWeekHeading(snapshot?.week ?? week)}
            </p>
          </div>
        ...
        {/* Week grid ≥768 — dates come from the anchor, counts from the API */}
        {pending ? (
          <div
            className="hidden md:grid md:grid-cols-7 md:gap-2"
            role="status"
            aria-busy="true"
          >
            <span className="sr-only">Loading this week’s deliveries</span>
            {week.days.map((date) => {
